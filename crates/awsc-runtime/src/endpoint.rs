...
pub fn context_params(
    ...
    let Some(awsc_model::Shape::Structure(shape)) = model.shape(&reference.target) else {
        return out;
    };
...
mod tests {
    ...
    fn reads_context_params_from_the_input() {
        ...
        let id = awsc_model::ShapeId::parse("com.x#Op").expect("shape id");
        let op = match model.shape(&id).expect("shape present") {
            awsc_model::Shape::Operation(op) => op.clone(),
