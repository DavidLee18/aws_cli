...
//! derivation.
//!
//! Both sides now build their operation names from `awsc_model::command_table`. This
...
use awsc_conformance::corpus::{custom_surface_path, customizations_path, models_dir};
use awsc_model::custom_surface::CustomSurface;
use awsc_model::customizations::Customizations;
use awsc_model::{command_table, Model};
...
fn command_table_matches_the_corpus_for_every_service() {
    ...
    let Ok(Some(corpus)) = awsc_conformance::corpus::Corpus::load_default() else {
    ...
    for entry in entries.flatten() {
        ...
        for typed in table.names.keys() {
            ...
            if awsc_model::customizations::is_event_stream_operation(&table.service, typed) {
                continue;
            }
