...
- **`emr create-default-roles` derives its service principal from the resolved endpoint.**
  Not from the region name and not hardcoded: it reads the suffix off the EMR endpoint, so
  the role trusts `elasticmapreduce.amazonaws.com` in the commercial regions and
  `elasticmapreduce.amazonaws.com.cn` in China. A wrong suffix creates a role nothing can
  assume, and nothing fails until a cluster refuses to start. Both host spellings are
  accepted (`elasticmapreduce.<region>.<suffix>` and `<region>.elasticmapreduce.<suffix>`),
  as the reference accepts both. Note the trust document also uses `2008-10-17`, and a
  single principal is a bare string where several are a list.
